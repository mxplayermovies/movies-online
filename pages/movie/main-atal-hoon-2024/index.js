...
const main_atal_hoon_2024 = ({ movie }) => {
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        const response = await fetch('/movies.json')
        const data = await response.json()
        const movie = data.find(movie => movie.id === 'INDEX117')
        setMovieData(movie)
      } catch (error) {
        console.error('Error fetching movie data:', error)
      }
  ...
  return (
    <div>
      ...
      >
        ...
        {/* <YouTubeBackground movieId="INDEX117" /> */}
        <DailyMotionBackground movieId='INDEX117' />
        <BackImage movieId='INDEX117' />
        <h1
...
export async function getServerSideProps () {
  ...
  const selectedMovie = data.find(movie => movie.id === 'INDEX117')
...
export default main_atal_hoon_2024;
