...
const captain_miller_2024 = ({ movie }) => {
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        const response = await fetch('/movies.json')
        const data = await response.json()
        const movie = data.find(movie => movie.id === 'INDEX50')
        setMovieData(movie)
      } catch (error) {
        console.error('Error fetching movie data:', error)
      }
  ...
  return (
    <div>
      <Head>
        ...
        />
       <meta property='og:image' content="https://res.cloudinary.com/dyx3mjea6/image/upload/v1711265111/Captain_Miller_2024_gqjxzv.jpg" />
