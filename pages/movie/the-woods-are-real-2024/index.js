...
const the_woods_are_real_2024 = ({ movie }) => {
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        const response = await fetch('/movies.json')
        const data = await response.json()
        const movie = data.find(movie => movie.id === 'INDEX102')
        setMovieData(movie)
      } catch (error) {
        console.error('Error fetching movie data:', error)
      }
...
export async function getServerSideProps () {
  ...
  const selectedMovie = data.find(movie => movie.id === 'INDEX102')
...
export default the_woods_are_real_2024
