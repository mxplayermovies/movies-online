...
const the_phantom_warrior_2024 = ({ movie }) => {
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        const response = await fetch('/movies.json')
        const data = await response.json()
        const movie = data.find(movie => movie.id === 'INDEX106')
        setMovieData(movie)
      } catch (error) {
        console.error('Error fetching movie data:', error)
      }
  ...
  return (
    <div>
      <Head>
        ...
        <meta
          property='og:image'
          content='https://res.cloudinary.com/dyx3mjea6/image/upload/v1711265127/The_Phantom_Warrior_2024_nvaobp.jpg'
        />
      ...
      >
        ...
        {/* <YouTubeBackground movieId="INDEX106" /> */}
...
export async function getServerSideProps () {
  ...
  const selectedMovie = data.find(movie => movie.id === 'INDEX106')
...
export default the_phantom_warrior_2024
