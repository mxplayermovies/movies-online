...
const bleeding_love_2024 = ({ movie }) => {
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        const response = await fetch('/movies.json')
        const data = await response.json()
        const movie = data.find(movie => movie.id === 'INDEX70')
        setMovieData(movie)
      } catch (error) {
        console.error('Error fetching movie data:', error)
      }
  ...
  return (
    <div>
      <Head>
        ...
        />
       <meta property='og:image' content="https://res.cloudinary.com/dyx3mjea6/image/upload/c_scale,w_1280,h_720,q_auto,f_auto,e_improve/e_sharpen:10/v1711265109/Argylle_2024_iwkv3h.jpg" />
...
export default bleeding_love_2024;
