  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        const response = await fetch('/movies.json')
        const data = await response.json()
        const movie = data.find(movie => movie.id === 'INDEX27')
        setMovieData(movie)
      } catch (error) {
        console.error('Error fetching movie data:', error)
      }
  ...
  return (
    <div>
      <Head>
        ...
        />
       <meta property='og:image' content="https://res.cloudinary.com/dyx3mjea6/image/upload/v1711265115/For_All_Mankind_S04_wmjayo.jpg" />
      ...
      >
        ...
        <DailyMotionBackground movieId="INDEX27" />
        <h1
...
<TabPanel>
  ...
  >
    {Array.from({ length: 10 }, (_, episodeNumber) => {
      ...
      const episodeUrl = movieData && movieData[episodeKey];
