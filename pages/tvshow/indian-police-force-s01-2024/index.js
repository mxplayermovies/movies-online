...
const indian_police_force_season_01_2024 = ({ movie }) => {
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        const response = await fetch('/movies.json')
        const data = await response.json()
        const movie = data.find(movie => movie.id === 'INDEX37')
        setMovieData(movie)
      } catch (error) {
        console.error('Error fetching movie data:', error)
      }
  ...
  return (
    <div>
      <Head>
        ...
        <meta
          property='og:image'
          content='https://i.postimg.cc/RC7kygRZ/Indian-Police-force-2024.jpg'
        />
      ...
      >
        ...
        <DailyMotionBackground movieId='INDEX37' />
        <h1
...
export default indian_police_force_season_01_2024
