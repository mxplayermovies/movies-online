...
const true_detective_s04_2024 = ({ movie }) => {
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        const response = await fetch('/movies.json')
        const data = await response.json()
        const movie = data.find(movie => movie.id === 'INDEX72')
        setMovieData(movie)
      } catch (error) {
        console.error('Error fetching movie data:', error)
      }
  ...
  return (
    <div>
      ...
      >
        ...
        <DailyMotionBackground movieId="INDEX72" />
        <h1
...
export default true_detective_s04_2024;
