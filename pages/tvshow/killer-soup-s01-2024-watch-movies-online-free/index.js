...
const killer_soup_season_01_2024 = ({ movie }) => {
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        const response = await fetch('/movies.json')
        const data = await response.json()
        const movie = data.find(movie => movie.id === 'INDEX36')
        setMovieData(movie)
      } catch (error) {
        console.error('Error fetching movie data:', error)
      }
  ...
  return (
    <div>
      <Head>
        ...
        <meta
          property='og:image'
          content='https://res.cloudinary.com/dyx3mjea6/image/upload/v1711265118/Killer_Soup_2024_orpjep.jpg'
        />
      ...
      >
        ...
        <DailyMotionBackground movieId='INDEX36' />
        <h1
...
export default killer_soup_season_01_2024
