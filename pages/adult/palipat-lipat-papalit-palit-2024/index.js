...
const palipat_lipat_papalit_palit_2024 = ({ movie }) => {
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        const response = await fetch('/movies.json')
        const data = await response.json()
        const movie = data.find(movie => movie.id === 'INDEX53')
        setMovieData(movie)
      } catch (error) {
        console.error('Error fetching movie data:', error)
      }
...
export default palipat_lipat_papalit_palit_2024;
